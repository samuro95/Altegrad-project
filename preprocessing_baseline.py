...
def random_walk(graph,node,walk_length, node2vec = False, p = None, q = None):
    walk = [node]
    if node2vec and p is not None and q is not None :
        for i in range(walk_length):
            ...
            if i == 0 :
                ...
                walk.append(np.random.choice(np.array(neighbors), p = normalized_probs))
    else :
        for i in range(walk_length):
